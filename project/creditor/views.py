...
class TransactionYearView(ListView):
   ...
   def get_context_data(self, **kwargs):
      ...
      context['receivables'] = self.get_queryset().filter(amount__lt=0).extra(select={'month': 'extract( month from stamp )'}).values('month').annotate(sum=Sum('amount')).order_by()
      context['income'] = self.get_queryset().filter(amount__gte=0).extra(select={'month': 'extract( month from stamp )'}).values('month').annotate(sum=Sum('amount')).order_by()
      context['year'] = self.param_year
