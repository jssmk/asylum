...
urlpatterns = [
    ...
    url(r'^transaction_table/(?P<year>[0-9]{4})/(?P<month>(1[0-2]|0[1-9]))/$', permission_required('is_staff')(views.TransactionMonthView.as_view()), name="table_m"),
    url(r'^transaction_table/(?P<year>[0-9]{4})/(?P<month>(1[0-2]|0[1-9]))/tag=(?P<tag>[0-9]+)$', permission_required('is_staff')(views.TransactionMonthView.as_view()), name="table_m"),
]
